...
const Cart = () => {
  ...
  return (
    <div className='mt-[100px] '>{/*cart */}
          ...
          <hr/>
          {
            food_list.map((item,index)=>{
              if(cartItems[item._id]>0){ // If the cartItem object has the item.id then return the page else be empty            
                return (
                <div key={index}>
                   <div  className='grid grid-cols-[1fr_1.5fr_1fr_1fr_1fr_0.5fr] items-center my-[10px] mx-[0px] text-black'>
      ...
      <div className="mt-[80px] flex flex-wrap md:flex-nowrap justify-between gap-[12vw] "> {/*card-bottom */}
        <div className="flex flex-col gap-[20px] w-full md:w-[50%]  "> {/*card-total*/}
          <h2>Card Total</h2>
          <div>
